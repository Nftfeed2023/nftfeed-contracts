...
async function main() {
    ...
    const tokenFactory = await getContractFactory("QuestPool");

    const tokenCt = await tokenFactory.deploy(
        TOKEN_ADDRESS.DOO_DOO
    );
    await tokenCt.deployed();

    console.log(`${"QuestPool"} deployed to:`, tokenCt.address);
    ...
    Object.assign(output, {
        ...
        verifyData: {
            ...
            contract: "contracts/QuestPool.sol:QuestPool"
        }
    });
