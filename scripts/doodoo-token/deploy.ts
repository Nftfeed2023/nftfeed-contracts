...
async function main() {
  ...
  const tokenFactory = await getContractFactory("DooDooToken");
  const tokenCt = await tokenFactory.deploy();
  await tokenCt.deployed();

  console.log(`${"DooDooToken"} deployed to:`, tokenCt.address);
  const linkDeploy =
  ...
  Object.assign(output, {
    ...
    verifyData: {
      ...
      contract: "contracts/DooDooToken.sol:DooDooToken",
    },
  });
