...
interface IRandomFactory {
    function randomRarity(address _erc721) external returns (uint256 rarity);
}

contract ERC721TemplateV1 is Ownable, Pausable, ERC721Enumerable {
    ...
    function setRandomFactory(address _randomFactory) external onlyOwner {
    ...
    function generateRarity(uint256 _tokenId) external {
        _exists(_tokenId);
        require(ownerOf(_tokenId) == _msgSender(), "Invalid owner");
        require(rarityOfTokenId[_tokenId] == 0, "Rarity initialized");
        require(randomFactory != address(0), "Generate rarity not start");
        rarityOfTokenId[_tokenId] = IRandomFactory(randomFactory).randomRarity(
            address(this)
        );
    }

    function getRarity(uint256 _tokenId) public view returns (uint256) {
        return rarityOfTokenId[_tokenId];
    }
}
